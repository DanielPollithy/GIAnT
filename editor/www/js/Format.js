	...
	else
	{
		...
		*/
		...
		else
		{
			...
			mxUtils.write(label, mxResources.get('data'));
			div.appendChild(label);
...
StyleFormatPanel.prototype.init = function()
{
	...
	*/
	...
	mxUtils.br(opsPanel);
	this.container.appendChild(this.addEditDataDialog(opsPanel));
	//this.container.appendChild(this.addLayerOps(opsPanel));
 ...
 * DANIEL
 */
StyleFormatPanel.prototype.addEditDataDialog = function(div) {
    var outer_div = div;
	...
	div.appendChild(generateEdgesBtn);
    ...
    // DANIEL
	nameInput.setAttribute('size', (mxClient.IS_QUIRKS) ? '6' : '10');
	nameInput.setAttribute('id', 'add_property_button');
	...
	nameInput.setAttribute('fire_button', addBtn.getAttribute('id'));
	...
	var applyBtn = mxUtils.button(mxResources.get('apply'), applyChanges);
		...
		try
		{
			...
			for (var i = 0; i < names.length; i++)
			{
				...
				else
				{
					...
					// DANIEL
                    if (CUSTOM_PROPERTY_CHANGE_HANDLERS.hasOwnProperty(names[i])) {
                        CUSTOM_PROPERTY_CHANGE_HANDLERS[names[i]](cell, cell_style, value, graph, names[i], texts[i].value, token_type);
                    }
 ...
 * DANIEL
 */
DiagramFormatPanel.prototype.addLayerOps = function(div)
{
	/*div.style.paddingTop = '10px';
	div.style.paddingBottom = '10px';
	...
	{
	    // DANIEL
	    var cell_style = graph.getCellStyle(selectionLayer);
	    console.log(cell_style);
		if (graph.isEnabled() && selectionLayer.value !== undefined && cell_style.locked !== 1)
		{
	...
	function refresh()
	{
		...
		function addLayer(index, label, child, defaultParent)
		{
			...
			if (label !== "Background") {
			    ...
			    graph.setCellStyles('locked', '1', [child]);
            }
